...
def make_heatmap_zs(datapath, sampling_file, featsfile, root_dir, task_name, model_root, patch_scaler_root, num_classes=2, gpu="0", norm_mean=[0.8201, 0.5207, 0.7189], norm_std=[0.1526, 0.1542, 0.1183]):
    ...
    for id in uid:
        ...
        model_name_list = os.listdir(os.path.join(model_root, task_name))
        model_name_list = [os.path.join(model_root, task_name, a) for a in model_name_list]

        patch_scaler_list = os.listdir(os.path.join(patch_scaler_root, task_name))
        patch_scaler_list = [os.path.join(patch_scaler_root, task_name, a) for a in patch_scaler_list]
        ...
        for i in range(len(model_name_list)):
            ...
            if len(gpu) > 1:
                ...
                net.load_state_dict(torch.load(model_name_list[i]))  # load the finetune weight parameters
            else:
                net = models.resnet18(pretrained=False, num_classes=num_classes).cuda()
                net.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(model_name_list[i]).items()})

            slidescore = slide_pred_vis_tumor(os.path.join(datapath, id), preprocess, net, featsfile, factor,
                                              patch_scaler_list[i])
            all_fold_score.append(slidescore)

        slidescore_mean = np.mean(np.stack((all_fold_score[0], all_fold_score[1], all_fold_score[2]), axis=0), axis=0)
        print('highest:',np.max(slidescore_mean) ,'lowest',np.min(slidescore[slidescore_mean!=-1]))
        print('slidescore calculation finished')

        slidemap = cv2.resize(slidescore_mean, dsize=None, fx=16, fy=16, interpolation=cv2.INTER_NEAREST)
        print('slide resize finished')

        savepath = os.path.join(root_dir, task_name, 'slide_heatmap', id)
        if not os.path.exists(savepath):
            os.makedirs(savepath)

        save_heatmap_file(os.path.join(datapath, id), savepath, slidemap)

        overlap = postprocess(slidemap)
